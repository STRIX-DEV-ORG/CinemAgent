<script lang="ts">
	...
	let semanticResults = $state<Array<{ source_type: string; source_id: string; chapter_id: string | null; content: string; metadata: Record<string, unknown>; distance: number; destination?: { kind: 'chapter' | 'node'; chapter_id: string | null; node_id?: string; node_type?: string } }>>([]);
	...
	async function indexCanon() {
		indexingCanon = true;
		try {
			const result = await workspaceApi.backfillSemanticIndex(params.graphId);
	...
	async function searchCanon() {
		...
		try {
			semanticResults = await workspaceApi.semanticSearch(params.graphId, semanticQuery);
		} catch (cause) {
	...
	function openSemanticResult(result: (typeof semanticResults)[number]) {
		...
		const node = destination?.kind === 'node' && destination.node_id ? `?node=${encodeURIComponent(destination.node_id)}` : '';
...
<main class="relative grid min-h-screen grid-cols-[3.5rem_minmax(0,1fr)_minmax(0,1fr)] bg-background">
	...
	<section class="flex min-h-0 flex-col">
		<header class="flex items-center justify-between border-b px-6 py-4">
			...
			<Button size="sm" variant="outline" disabled={intelligenceLoading} onclick={() => void loadIntelligence()}>Story intelligence</Button>
			<Button
				size="sm"
				variant="outline"
		...
		{#if intelligenceOpen}<aside class="max-h-[46vh] overflow-y-auto border-b bg-muted/30 p-4">
				<div class="mb-4 flex items-center justify-between gap-3">
					<div><p class="text-xs font-semibold tracking-wide text-primary uppercase">Story intelligence</p><p class="text-sm text-muted-foreground">Live ClickHouse insight models for this narrative.</p></div>
					<Button size="sm" variant="ghost" onclick={() => (intelligenceOpen = false)}>Close results</Button>
				</div>
				{#if intelligenceLoading}<p class="text-sm text-muted-foreground">Refreshing narrative intelligence…</p>{:else}<div class="grid gap-4 lg:grid-cols-2">
					<section class="rounded-md border bg-background p-3"><div class="flex items-center justify-between"><h3 class="font-semibold">Story health</h3><Button size="sm" variant="ghost" disabled={indexingCanon} onclick={() => void indexCanon()}>{indexingCanon ? 'Indexing…' : 'Index canon'}</Button></div>
						{#if health}<dl class="mt-3 grid grid-cols-3 gap-3 text-sm"><div><dt class="text-muted-foreground">Chapters</dt><dd class="font-semibold">{health.chapters}</dd></div><div><dt class="text-muted-foreground">Nodes</dt><dd class="font-semibold">{health.nodes}</dd></div><div><dt class="text-muted-foreground">Relations</dt><dd class="font-semibold">{health.relations}</dd></div><div><dt class="text-muted-foreground">Open assumptions</dt><dd class="font-semibold">{health.unresolved_assumptions}</dd></div><div><dt class="text-muted-foreground">Evidence</dt><dd class="font-semibold">{health.evidence_count}</dd></div><div><dt class="text-muted-foreground">Coverage</dt><dd class="font-semibold">{Math.round(health.evidence_coverage * 100)}%</dd></div></dl>{/if}
					</section>
					<section class="rounded-md border bg-background p-3"><h3 class="font-semibold">Semantic canon</h3><form class="mt-3 flex gap-2" onsubmit={(event) => { event.preventDefault(); void searchCanon(); }}><Input bind:value={semanticQuery} placeholder="Find prior canon…" /><Button size="sm" type="submit">Search</Button></form>
						{#if semanticResults.length}<div class="mt-3 grid gap-2">{#each semanticResults as result (result.source_type + result.source_id)}<button class="cursor-pointer rounded border p-2 text-left text-sm transition-colors hover:border-primary hover:bg-muted" onclick={() => openSemanticResult(result)}><p class="font-medium">{result.source_type} · {Math.round((1 - result.distance) * 100)}% match</p><p class="mt-1 line-clamp-3 text-muted-foreground">{result.content}</p><p class="mt-2 text-xs text-primary">Open {result.destination?.kind === 'node' ? 'node' : 'chapter'} →</p></button>{/each}</div>{/if}
					</section>
					<section class="rounded-md border bg-background p-3"><h3 class="font-semibold">Character presence</h3>{#if presence.length}<div class="mt-2 grid gap-1 text-sm">{#each presence.slice(0, 12) as item (item.chapter_id + item.entity_id)}<button class="cursor-pointer rounded px-1 py-1 text-left hover:bg-muted" onclick={() => goto(resolve(`/stories/${params.graphId}/chapters/${item.chapter_id}`))}>{item.name} <span class="text-muted-foreground">in {item.chapter_sequence}. {item.chapter_title}</span></button>{/each}</div>{:else}<p class="mt-2 text-sm text-muted-foreground">Link character entities to chapters to see their presence.</p>{/if}</section>
					<section class="rounded-md border bg-background p-3"><h3 class="font-semibold">Relationship evolution</h3>{#if relationTimeline.length}<div class="mt-2 grid gap-1 text-sm">{#each relationTimeline.slice(0, 12) as relation (relation.id + relation.chapter_id)}<button class="cursor-pointer rounded px-1 py-1 text-left hover:bg-muted" onclick={() => goto(resolve(`/stories/${params.graphId}/chapters/${relation.chapter_id}`))}>{relation.label.replaceAll('_', ' ')} <span class="text-muted-foreground">· {relation.chapter_sequence}. {relation.chapter_title}</span></button>{/each}</div>{:else}<p class="mt-2 text-sm text-muted-foreground">Create chapter relations to see their evolution.</p>{/if}</section>
					<section class="rounded-md border bg-background p-3 lg:col-span-2"><h3 class="font-semibold">ClickHouse activity</h3><p class="mt-1 text-sm text-muted-foreground">{metrics?.activity.reduce((total, item) => total + item.event_count, 0) ?? 0} immutable narrative events · {metrics?.embeddings.reduce((total, item) => total + item.count, 0) ?? 0} indexed canon records</p>{#if metrics?.agent_runs.length}<p class="mt-2 text-xs text-muted-foreground">Agent runs: {metrics.agent_runs.map((item) => `${item.count} ${item.agent_group} ${item.status}`).join(' · ')}</p>{/if}</section>
					<section class="rounded-md border bg-background p-3 lg:col-span-2"><h3 class="font-semibold">Narrative history</h3>{#if recentEvents.length}<div class="mt-2 grid gap-1 text-xs text-muted-foreground">{#each recentEvents.slice(0, 8) as event (event.event_id)}<p><span class="font-medium text-foreground">{event.event_type.replaceAll('_', ' ')}</span> · {event.actor_type} · {new Date(event.occurred_at).toLocaleString()}</p>{/each}</div>{:else}<p class="mt-2 text-sm text-muted-foreground">No immutable history has been recorded for this story yet.</p>{/if}</section>
				</div>{/if}
			</aside>{/if}
