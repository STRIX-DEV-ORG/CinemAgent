...
-- Immutable stage ledger.  ``agent_run`` remains the inexpensive current-state
-- read model; this table makes retries, worker restarts, and audit history
-- observable without relying on a JSON blob in that read model.
CREATE TABLE IF NOT EXISTS agent_run_stage
(
    run_id UUID,
    graph_id UUID,
    chapter_id Nullable(UUID),
...
ENGINE = MergeTree()
ORDER BY (graph_id, run_id, attempt, name, occurred_at);


-- ClickHouse-backed cooperative lease/fence for the stateless worker.  The
-- workload is deliberately at-least-once; stage and artifact idempotency keys
...
ENGINE = MergeTree()
ORDER BY (graph_id, run_id, attempt, claimed_at);
